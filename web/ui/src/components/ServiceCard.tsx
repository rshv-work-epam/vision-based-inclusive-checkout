    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <dl className="mt-3 grid grid-cols-3 text-center text-sm">
        <div>
          ...
          <dd className="font-semibold">{formatStatus(status?.healthz)}</dd>
        </div>
        <div>
          <dt className="text-slate-500">/readyz</dt>
          <dd className="font-semibold">{formatStatus(status?.readyz)}</dd>
        </div>
        <div>
          <dt className="text-slate-500">/livez</dt>
          <dd className="font-semibold">{formatStatus(status?.livez)}</dd>
        </div>
      </dl>
    </div>
  )
